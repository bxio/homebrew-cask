cask 'cd-to-hyper' do
  ...
  app "cd_to_#{version.major_minor.dots_to_underscores}/hyper/cd_to.app", target: 'cd_to_hyper.app'

  caveats <<~EOS
    To complete installation:
    1. Open the the Applications folder in finder.
    2. Drag "cd_to_hyper.app" onto the Finder toolbar while holding down the command(⌘) and option(⌥) keys.

    To use, just click on the new button and instantly opens a new Terminal.app window.

    Only "cd_to.app" for Hyper.app has been added to the Applications folder. The iterm and terminal versions are also staged.
  EOS
end
